...
-- Target: Fresh/Existing Supabase Database
...
DROP POLICY IF EXISTS "Users can view own stock movements" ON public.product_stock_movements;
CREATE POLICY "Users can view own stock movements"
  ON public.product_stock_movements FOR SELECT
  USING (auth.uid() = user_id);

DROP POLICY IF EXISTS "Users can insert own stock movements" ON public.product_stock_movements;
...
DROP POLICY IF EXISTS "Users can view own income items" ON public.income_items;
CREATE POLICY "Users can view own income items"
  ON public.income_items FOR SELECT
  USING (auth.uid() = user_id);

DROP POLICY IF EXISTS "Users can insert own income items" ON public.income_items;
CREATE POLICY "Users can insert own income items"
  ON public.income_items FOR INSERT
  WITH CHECK (auth.uid() = user_id);

DROP POLICY IF EXISTS "Users can update own income items" ON public.income_items;
CREATE POLICY "Users can update own income items"
  ON public.income_items FOR UPDATE
  USING (auth.uid() = user_id);

DROP POLICY IF EXISTS "Users can delete own income items" ON public.income_items;
...
CREATE INDEX IF NOT EXISTS idx_stock_movements_user_id ON public.product_stock_movements(user_id);
